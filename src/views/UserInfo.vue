      ...
      <div v-show="isEdit" class="tcommonBox">
        ...
        <section>
            ...
            <li class="avatar-list">
              ...
              <el-upload
                ...
                <img
                    ...
                    :src="userInfoObj.avatar?userInfoObj.avatar:'../assets/img/tou.jpg'"
          ...
          <div class=" save-info-btn">
            <a class="tcol-irs--bg" href="javascript:void(0);" @click="isEdit=!isEdit">返 回</a>
...
let store = useStore()
let uploadURL = store.state.baseURL + 'upload';
...
function routeChange() {
  if (localStorage.getItem('userInfo')) {
    ...
    getUserInfo(userInfo.id).then((response) => {
      userInfoObj = reactive(response);
      userInfoObj.head_start = 0;
    })
  } else {
...
routeChange() // 执行展示页面信息

// 保存编辑的用户信息
function saveInfoFun() {
  // 昵称为必填
  ...
  // 保存信息接口，返回展示页
  savaUserInfo(userInfoObj).then((response) => {
...
function handleAvatarSuccess(res, file) {
  if (res.code === 200) {
    userInfoObj.avatar = res.data;
    userInfoObj.head_start = 1;
  } else {
...
// 如果路由有变化，会再次执行该方法
watch(store, routeChange)

</script>

<style>
.userInfoBox .avatar-list {
  position: relative;
}
